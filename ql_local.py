...
def ql_local(self, l, sample):

	# Define the box dimensions
	if sample.pbc:
		Lx = sample.boundaries[0][1] - sample.boundaries[0][0]
		Ly = sample.boundaries[1][1] - sample.boundaries[1][0]
		if self.dim==3:
			Lz = sample.boundaries[2][1] - sample.boundaries[2][0]
		else:
			Lz = None
	...
	for j in self.neighs:
		if sample.pbc:
			d = np.array(vecPBC([self.x, self.y, self.z], [sample.centers[j].x, sample.centers[j].y, sample.centers[j].z], self.boundaries), dtype=np.float)

		else:
			d = np.array([self.x, self.y, self.z], dtype=np.float) - np.array([sample.centers[j].x, sample.centers[j].y, sample.centers[j].z], dtype=np.float)	
	...
	if len(self.neighs) != 0:
		...
		ql = [ql_i, qlmbar, qlmtilde, qlmtilde_neighs]
